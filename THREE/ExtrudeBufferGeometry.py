...
class ExtrudeBufferGeometry(BufferGeometry):
    def __init__(self, shapes=None, options=None):
        ...
        self.addShapeList(shapes, options)
    ...
    def addShape(self, shape, options ):
        arrays = options['arrays'] if 'arrays' in options else self.getArrays()
        verticesArray = arrays['position']
        indicesArray = arrays['index']
        uvArray = arrays['uv']

        placeholder = []

        amount = options['amount'] if 'amount' in options is not None else 100
        ...
        if extrudePath:
            ...
            splineTube = options['frames'] if 'frames' in options else extrudePath.computeFrenetFrames( steps, False )
        ...
        def f4( a, b, c, d, wallContour, stepIndex, stepsLength, contourIndex1, contourIndex2 ):
            addVertex( a )
            addVertex( b )
            addVertex( d )
            ...
            nextIndex = int(len(verticesArray) / 3)
            uvs = uvgen.generateSideWallUV( self, verticesArray, nextIndex - 6, nextIndex - 3, nextIndex - 2, nextIndex - 1 )
        ...
        def buildLidFaces():
            ...
            self.addGroup( start, len(verticesArray)/3 -start, options['material'] if 'material' in options else 0)
        ...
        if 'arrays' not in options:
            self.setIndex( indicesArray )
